...
function Navbar() {
  ...
  const categories =["hotdeal",...new Set(data.map((list)=>{  
  ...
  return (<div className='border'>
    <div className='  section__margin'>
      <div className='deals__navbar'>
          <div>
              <h2 className='deals__navbar-title'>Exclusive <span className='deals__navbar-span'>Deals</span> </h2>  
           </div> 
         <div className='deals__navbar__container'>
              {categories.map((data)=>(
                <p onClick={()=>{
                  ...
                  setClick(data)
                  setPosition(0)
                }} key={data} className={data === click? "deals__navbar__container-active" :""}>{data}</p>
              ))}
          </div>
        ...
        <div className='deals__margin deals__container__flex'  >
          {datas.map(({id,category,img,title,detail})=>{
                return <div className='deals__container' style={cardStyle}  key={id}>
                    ...
                    <div className='deals__conatiner__detainscontainer'>
                       ...
                       <div className='detals__container__category-padding'>
                          <h3 className='detals__container__category-padding__title'>{title}</h3>
                          <p className='deals__conatiner__category__details'>{detail}</p>
                       </div>
                    </div>
                </div>
          }) }
