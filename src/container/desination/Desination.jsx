...
import {DesinationRight,DesinationLeft} from "../../components"
const category = [...new Set(desination.map((data)=>data.category))]
const desinationData = desination.filter((data)=>data.category ==='all destinations')
        ...
        <div className='desination__container-main'>
          <div className='desination__container-navbar'>
             <div className='desination__container-links'>
              <div>
              <h2 className='desination__navbar-title'>Popular <span>Destinations</span></h2>
            </div>
              {category.map((list)=>(
                <p onClick={()=>handleClick(list)} key={list} className={list === click ?'desination__navbar-active':""}>{list}</p>
              ))}
          </div>
